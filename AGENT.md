...
> 구현·문서화·발표 준비 시에는 **§11 서면 평가기준(30점)**도 함께 확인한다.
...
## 11. 오픈소스 개발자대회 1차(서면) 평가기준 — 총 30점

> 이 절은 대회 안내자료의 평가표를 AI 에이전트가 구현·리뷰·문서 작업에 반영할 수 있도록 옮긴 것이다.
> **평가 대상 자료는 결과보고서, 소스코드, 시연 영상**이다.
> 기존 제품 범위와 역할 경계를 깨면서 점수만 좇지 않는다. 충돌 시 문서 상단의 우선순위를 따른다.

| 기준 | 평가항목 | 배점 | AI 작업 시 확인할 것 |
|---|---|---:|---|
| 프로젝트 구조 및 코드 완성도 | 코드의 가독성, 주석의 활용도, 구조의 합리성. 코드가 목적에 맞게 제 기능을 하는 정도 | 6 | 이름과 모듈 경계를 명확히 하고, 필요한 곳에 이유를 설명하는 주석을 남긴다. 정상 경로뿐 아니라 오류·경계 조건을 테스트하고 실제 실행 가능 상태를 유지한다. |
| 오픈소스 프로젝트로의 발전 가능성 | 출품작이 오픈소스 프로젝트로 진행 중이거나 향후 발전할 수 있는 가능성 | 6 | 설치·사용·기여 경로와 확장 지점을 명확히 한다. 특정 데모에만 맞춘 일회성 구현을 피하고 재사용성과 유지보수성을 확인한다. |
| 개발 문서의 구체성 | 개발 문서가 구체적이며 쉽게 이해할 수 있고 목적에 맞게 작성된 정도 | 6 | 명령어, 입력·출력, 설정, 제약, 검증 방법을 재현 가능한 수준으로 기록한다. 코드와 문서가 어긋나지 않는지 함께 검증한다. |
| 프로젝트 혁신성 | 프로젝트의 기술력과 기술적 수준(최신 기술 활용 정도) | 6 | APrIce의 차별점인 **머지 전 정적 분석 기반 API 비용 예측·예산 차단**이 코드와 시연에서 분명히 드러나게 한다. 새 기술은 문제 해결에 기여할 때만 도입하고 근거를 문서화한다. |
| 프로젝트 팀워크 | (팀) 팀 역할 분배와 운영에서 보이는 팀워크 수준. (개인) 프로젝트 관리체계가 얼마나 안정적이고 체계적인지 나타내는 수준. GitHub Issues, review, pull requests, commit, merge, 커뮤니티 등 관리체계 | 6 | `ROLES.md`의 소유권을 지키고, 작업 단위·결정 근거·리뷰 가능성을 기록으로 남긴다. 이슈·PR·커밋은 목적과 범위가 드러나게 관리하고 공동 자산 변경 절차를 준수한다. |
| **합계** |  | **30** | 다섯 기준이 결과보고서·소스코드·시연 영상에서 서로 일관되게 확인되어야 한다. |

### 11.1 완료 판단에 적용하는 방법

기능을 “완료”로 보고하기 전에 해당 작업 범위에서 다음을 확인한다.

1. **완성도** — 빌드·테스트·실행 검증 근거가 있고, 읽기 어려운 구조나 불필요한 복잡성이 없는가?
2. **발전 가능성** — 다른 사용자가 설치·사용·확장할 수 있는 형태인가?
3. **문서성** — 처음 보는 사람이 문서만으로 목적과 사용법, 제약을 이해하고 재현할 수 있는가?
4. **혁신성** — 기존 사후 비용 관측 도구와 다른 APrIce의 사전 예측·차단 가치가 드러나는가?
5. **팀워크** — 역할 경계와 공동 변경 절차를 지켰고, 변경 이유를 리뷰 가능한 기록으로 남겼는가?

작업 결과를 요약할 때는 관련된 평가기준과 검증 근거를 함께 언급한다. 단, 사소한 수정에 다섯 기준을 억지로 모두 적용하지 않는다.
